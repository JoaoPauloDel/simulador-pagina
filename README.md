...
##  Metodologia
O simulador foi integralmente desenvolvido na linguagem de programação Java, operando com uma arquitetura que separa a lógica de negócio da interface visual em três classes distintas: `Main.java` (ponto de entrada), `Algoritmos.java` (lógica pura dos algoritmos, sem dependência de UI) e `Interface.java` (interface gráfica). A Interface Gráfica foi construída com o pacote nativo `javax.swing`, projetada com um design moderno escuro. O programa recebe via interface uma sequência de números inteiros representando as chamadas de página, além da capacidade de frames na memória. O sistema processa as requisições iterando sobre estruturas de dados específicas: filas (`Queue`) para o FIFO, `LinkedHashSet` para o LRU e arrays com bits de controlo e ponteiro circular para o algoritmo do Relógio.
...
**
##  Conclusão
O desenvolvimento do simulador cumpriu o seu objetivo pedagógico e técnico, ilustrando com exatidão matemática o impacto das políticas de substituição de memória. Com os resultados obtidos através da representação gráfica, consolida-se o entendimento de que algoritmos puramente temporais de chegada (como o FIFO) são subótimos, enquanto estratégias focadas no histórico recente de uso (LRU e Relógio) entregam uma relação custo-benefício excelente para arquiteturas reais de computadores. A separação da lógica em classes distintas garantiu organização e facilidade de manutenção do código.
